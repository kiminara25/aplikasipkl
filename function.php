...
if(isset($_POST['barangmasuk'])){
    ...
    $insertb = mysqli_query($c,"insert into masuk (idproduk,qty) value('$idproduk','$qty')");
...
if(isset($_POST['editdatabarangmasuk'])){
    ...
    //cari tau qtynya sekarang berapa
...
if(isset($_POST['hapusorder'])){
    ...
    while($ok=mysqli_fetch_array($cekdata)){
        ...
        // Mencari tahu stock sekarang berapa
        $caristock = mysqli_query($c,"select * from produk where idproduk='$idproduk'");
        $caristock2 = mysqli_fetch_array($caristock);
        $stocksekarang = $caristock2['stock'];
...
$c = mysqli_connect('localhost','root','','kasir');
    ...
    $cek_user = mysqli_query($c,"SELECT * FROM user WHERE username = '$username'");
    ...
    if ($cek_login > 0) {
        ...
        window.location = 'registrasi.php';
            ...
            mysqli_query($c,"INSERT INTO USER VALUES('','$nama','$username','$password1')");
            echo "<script>
